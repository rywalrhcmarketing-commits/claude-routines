...
CLS=$1; shift
rm -rf /tmp/kt-out && mkdir -p /tmp/kt-out
java -cp "$K/kotlin-compiler.jar:$SP/khome/lib/annotations-13.0.jar" \
  ...
  -d /tmp/kt-out -nowarn "$@" 2>&1 | grep -v JAVA_TOOL | grep "error:" && exit 1
java -cp "/tmp/logstub:/tmp/kt-out:$K/kotlin-stdlib.jar:$K/coroutines.jar:$K/junit-4.13.2.jar:$K/hamcrest-core-1.3.jar:$K/android-all.jar" \
