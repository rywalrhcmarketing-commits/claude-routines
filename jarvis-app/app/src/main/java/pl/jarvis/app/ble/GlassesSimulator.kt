 ...
 * aktualizacja stanu w [JarvisManager], reakcja UI i cała warstwa AI działają
...
class GlassesSimulator(
    ...
    /**
     * Przyjmuje komendę wysłaną przez [JarvisManager] i odgrywa reakcję okularów.
    ...
    suspend fun fileBytes(filename: String): ByteArray {
        ...
        if (filename !in files) {
            throw JarvisException("Symulator: brak pliku $filename")
        }
        return when {
 ...
 * i na emulatorze [pl.jarvis.app.ble.CanvasPhotoSource] rysuje czytelne sceny.
 */
