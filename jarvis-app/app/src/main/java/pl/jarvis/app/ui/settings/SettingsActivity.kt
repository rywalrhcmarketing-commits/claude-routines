        ...
        .let { contract ->
            registerForActivityResult(contract) { result ->
                if (result.resultCode == android.app.Activity.RESULT_OK) {
                    val app = application as pl.jarvis.app.JarvisApplication
            ...
            JarvisTheme {
                SettingsScreen(
                    ...
                    onRequestGoogleSignIn = {
                        try {
                            val gcalService = pl.jarvis.app.calendar.GoogleCalendarService(this@SettingsActivity)
        ...
        Column(
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(12.dp)) {
                    ...
                    TextButton(
                        onClick = {
                            pl.jarvis.app.JarvisApplication.get().settings.resetOnboarding()
                            val intent = android.content.Intent(
                                context,
                                pl.jarvis.app.ui.onboarding.OnboardingActivity::class.java
                            )
...
private fun ModelSection(
    ...
    val models = remember(providerId) { pl.jarvis.app.data.ModelRegistry.forProvider(providerId) }
    val selectedInfo = selectedModelId?.let { pl.jarvis.app.data.ModelRegistry.findById(it) }
    val defaultInfo = pl.jarvis.app.data.ModelRegistry.defaultFor(providerId)
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(12.dp)) {
                    ...
                    if (info.deprecated && info.replacementId != null) {
                        val replacement = pl.jarvis.app.data.ModelRegistry.findById(info.replacementId)
                        Text(
...
private fun VoiceInstallGuideSection() {
    val audio = remember { pl.jarvis.app.JarvisApplication.get().audio }
    var expanded by remember { mutableStateOf(false) }
...
private fun PersonaSection(
    ...
    var expanded by remember { mutableStateOf(false) }
    val personas = remember { pl.jarvis.app.persona.PersonaRegistry.all() }
    val currentPersona = personas.find { it.id == selectedPersonaId }
        ?: if (selectedPersonaId == "custom") pl.jarvis.app.persona.PersonaRegistry.customFromPrompt(customPrompt)
        else pl.jarvis.app.persona.PersonaRegistry.default()
...
private fun VoiceSection(
    ...
    currentVoice: pl.jarvis.app.audio.VoiceInfo?,
    speechRate: Float,
    pitch: Float,
...
private fun ProactiveAlertsSection() {
    val context = LocalContext.current
    val app = pl.jarvis.app.JarvisApplication.get()
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(12.dp)) {
                ...
                Spacer(Modifier.size(4.dp))
                Text(
                    "Jarvis sprawdza kalendarz i pogodę co 15 min. " +
        ...
        if (enabled) {
            ...
            TextButton(
                onClick = {
                    ...
                    val request = androidx.work.OneTimeWorkRequestBuilder<pl.jarvis.app.proactive.ProactiveAlertsWorker>().build()
...
private fun ActionsSection() {
    val context = LocalContext.current
    val audio = remember { pl.jarvis.app.JarvisApplication.get().audio }
    val executor = remember { pl.jarvis.app.actions.ActionExecutor(context) }
    val detector = remember { pl.jarvis.app.actions.SmartActionDetector() }
    var installedApps by remember { mutableStateOf<List<pl.jarvis.app.actions.AppInfo>>(emptyList()) }
    val directExecutor = remember { pl.jarvis.app.actions.DirectActionExecutor(context) }
    ...
    var actionMode by remember {
        mutableStateOf(pl.jarvis.app.JarvisApplication.get().settings.getActionMode())
    }
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(12.dp)) {
                Text(
                    "🎯 Jarvis może wykonywać akcje w Twoim imieniu",
                    style = MaterialTheme.typography.titleSmall,
                    fontWeight = androidx.compose.ui.text.font.FontWeight.Bold
                )
                Spacer(Modifier.size(4.dp))
            ...
            Column(modifier = Modifier.padding(12.dp)) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    RadioButton(
                        ...
                        onClick = {
                            actionMode = "SAFE"
                            pl.jarvis.app.JarvisApplication.get().settings.setActionMode("SAFE")
                        }
                    )
                    Spacer(Modifier.size(4.dp))
                    Column {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    RadioButton(
                        ...
                        onClick = {
                            actionMode = "DIRECT"
                            pl.jarvis.app.JarvisApplication.get().settings.setActionMode("DIRECT")
                        }
                    )
                    Spacer(Modifier.size(4.dp))
                    Column {
                        ...
                        Button(
                            onClick = {
                                permissionLauncher.launch(
                                    pl.jarvis.app.actions.DirectActionExecutor.REQUIRED_PERMISSIONS
                                )
                            },
                            modifier = Modifier.fillMaxWidth()
                        ) {
...
private fun WakeWordSection(
    ...
    val current = wakeWords.find { it.id == selectedId } ?: pl.jarvis.app.data.WakeWordRegistry.default()
...
private fun WakeWordItem(
    ww: pl.jarvis.app.data.WakeWord,
    selectedId: String,
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            Row(
                ...
                Switch(
                    ...
                    onCheckedChange = { newVal ->
                        ...
                        pl.jarvis.app.JarvisApplication.get()
                            .orchestrator
                            .let { orch ->
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(
                        "Docelowy język: ${pl.jarvis.app.translation.SimultaneousTranslator.languageName(translationTarget)}",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
            ...
            ExposedDropdownMenuBox(
                ...
                OutlinedTextField(
                    value = pl.jarvis.app.translation.SimultaneousTranslator.languageName(translationTarget),
                    onValueChange = {},
                    readOnly = true,
                ...
                ExposedDropdownMenu(
                    ...
                    pl.jarvis.app.translation.SimultaneousTranslator.SUPPORTED_LANGUAGES.forEach { (code, _) ->
                        DropdownMenuItem(
                            text = { Text(pl.jarvis.app.translation.SimultaneousTranslator.languageName(code)) },
                            onClick = {
                        ...
                        if (gcalConnected) {
                            OutlinedButton(
                                onClick = {
                                    scope.launch {
                                        try {
                                            pl.jarvis.app.calendar.GoogleCalendarService(context).signOut()
...
private fun CaptureModeSection() {
    val context = LocalContext.current
    val settings = remember { (context.applicationContext as pl.jarvis.app.JarvisApplication).settings }
    var preferredMode by remember { mutableStateOf(pl.jarvis.app.ai.CaptureMode.valueOf(settings.getPreferredCaptureMode())) }
    var autoDegrade by remember { mutableStateOf(settings.isAutoDegradeCaptureEnabled()) }
    var providerCaps by remember { mutableStateOf<pl.jarvis.app.ai.ProviderCapabilities?>(null) }

    LaunchedEffect(Unit) {
        try {
            val app = context.applicationContext as pl.jarvis.app.JarvisApplication
            val providerId = app.settings.getActiveProvider()
            providerCaps = pl.jarvis.app.ai.AIProviderFactory.getCapabilitiesFor(providerId)
        } catch (e: Exception) { }
    }
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            pl.jarvis.app.ai.CaptureMode.values().forEach { mode ->
...
private fun PowerModeSection() {
    ...
    val powerManager = remember { pl.jarvis.app.power.PowerManager(context, settings) }
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            pl.jarvis.app.power.PowerMode.values().forEach { mode ->
                Row(
                    verticalAlignment = Alignment.CenterVertically,
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(8.dp)) {
                    ...
                    Text("• Wake word \"Jarvis Start\" - ~3%/h", style = MaterialTheme.typography.labelSmall)
